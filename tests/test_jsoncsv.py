...
class Testjsoncsv(unittest.TestCase):
    ...
    def test_jsoncsv_expand_restore(self):
        ...
        result = runner.invoke(
            jsoncsv, args=["-e", "fixture/files/raw.2.json", "fixture/files/tmp.expand.2.json"]
        )
        assert result.exit_code == 0
        result = runner.invoke(
            jsoncsv,
            args=["-r", "fixture/files/tmp.expand.2.json", "fixture/files/tmp.restore.2.json"],
        )
        assert result.exit_code == 0
    ...
    def test_jsoncsv_with_error_args(self):
        runner = CliRunner()
        args = ["-s", "aa", "-e", "fixture/files/raw.0.json", "fixture/files/tmp.expand.0.json"]
        result = runner.invoke(jsoncsv, args=args)
        assert result.exit_code != 0

    def test_jsoncsv_with_error_sep_args(self):
        runner = CliRunner()
        args = ["-s", "\\", "-e", "fixture/files/raw.0.json", "fixture/files/tmp.expand.0.json"]
        result = runner.invoke(jsoncsv, args=args)
        assert result.exit_code != 0

    def test_jsoncsv_with_error_args_expand_and_restore(self):
        runner = CliRunner()
        args = ["-r", "-e", "fixture/files/raw.0.json", "fixture/files/tmp.expand.0.json"]
        result = runner.invoke(jsoncsv, args=args)
        assert result.exit_code != 0
